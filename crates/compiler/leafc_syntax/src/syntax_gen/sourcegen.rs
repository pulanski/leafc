...
fn update_syntax_kinds_if_necessary() {
    ...
    utils::ensure_file_contents(syntax_kinds_path.as_path(), &new_syntax_kinds);
}

fn create_syntax_kinds() -> (String, PathBuf) {
...
fn sourcegen_ast() {
    ...
    let ast_tokens_file = utils::project_root().join(TOKENS_FILE);
    utils::ensure_file_contents(ast_tokens_file.as_path(), &ast_tokens);

    let ast_nodes = generate_nodes(KINDS_SRC, &ast);
    let ast_nodes_file = utils::project_root().join(NODES_FILE);
    utils::ensure_file_contents(ast_nodes_file.as_path(), &ast_nodes);
}
...
fn generate_nodes(kinds: SyntaxKinds<'_>, grammar: &AstSrc) -> String {
    ...
    let res = utils::add_preamble(utils::reformat(res), GeneratorKind::SyntaxNode);
    res.replace("#[derive", "\n#[derive")
}
...
fn generate_syntax_kinds(grammar: SyntaxKinds<'_>) -> String {
    ...
    let ast = quote! {
        ...
        pub enum SyntaxKind {
            ...
            #[doc(hidden)]
            EOF,
            #[doc = "Literals (e.g. RUNE, STRING, RAW_STRING, INTEGER, FLOAT, LIFETIME)"]
            #(#literals,)*
            #[doc = "Tokens (e.g. WHITESPACE, ERROR, IDENTIFIER, COMMENT, DOC_COMMENT)"]
            #(#tokens,)*
            // TODO: figure out how to integrate this with the docs
            // #(doc_comment!(include_str!(concat!("../../../../docs/src/chapter_1/tokens/", #tokens, ".md"))), #tokens,)*
            #[doc = "Keywords (e.g. AS, BREAK, CONST, ENUM, EXTERN, FALSE, FN, IF, IMPL, IN, LET, LOOP, MATCH, MOD, MOVE, MUT, PUB, REF, RETURN, SELF, STATIC, STRUCT, SUPER, TRUE, TRAIT, TYPE, UNSAFE, USE, WHERE, WHILE)"]
            #(#all_keywords,)*
            #[doc = "Punctuation (e.g. DOT, COMMA, SEMICOLON, COLON, COLONCOLON, EQ, EQEQ, FAT_ARROW, LARROW, RARROW, DOLLAR, LPAREN, RPAREN, LBRACK, RBRACK, LBRACE, RBRACE)"]
            #(#punctuation,)*
            #[doc = "Nodes (e.g. VarDecl, etc.)"]
            #(#nodes,)*
