...
// Concern is deeply nested pairs could be exponentially more expensive than
// their inner dupes.
...
mod tests {
    ...
    fn test_dupe_() {
        ...
        impl<T> Clone for FooT<T> {
            fn clone(&self) -> Self {
                FooT { foo: self.foo.clone() }
            }
        }

        let x = FooT { foo: Arc::new(NoClone()) };
        assert_eq!(x, x.dupe());
    }
    ...
    fn test_dupe_enum() {
        ...
        assert_eq!(x, x.dupe());
        let x = Qux::Baz { foo: Foo(), bar: Bar(7, false, Foo2), baz: Baz { foo: 9 } };
        assert_eq!(x, x.dupe());
    }
